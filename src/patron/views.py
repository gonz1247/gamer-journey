...
def profile_view(request):
    ...
    if user.is_authenticated:
        ...
        if current_diary:
            ...
            game_info = Game.game_id_search(
                current_diary.values_list("game_id", flat=True),
                fields="name,genres.*,themes.*,cover.*",
            )
            ...
            for info in game_info:
                current_diary_and_info.append(
                    current_diary.get(game_id=info["game_id"]).__dict__ | info
                )
